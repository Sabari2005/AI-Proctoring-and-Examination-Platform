"""Exam screen UI for section navigation, answer capture, and runtime monitoring.

Supports classic and server-driven exam flows, including optional coding mode,
background proctor-alert polling, and asynchronous answer persistence.
"""
...
class ProctorAlertsWorker(QThread):
    ...
    alerts_received = pyqtSignal(list)
    violation_count_updated = pyqtSignal(int)
...
class ExamScreen(QWidget):
    ...
    def __init__(
        ...
        self._alert_worker_in_flight = False
    ...
    def start_exam(self, exam_data: dict = None):
        ...
        # Engage security / proctoring now that the data is loaded
        if self._proctoring_service:
            try:
                self._proctoring_service.start_exam_monitoring(engage_security=True)
            except Exception as e:
    ...
    def _resolve_question_type(question: dict) -> str:
        ...
        aliases = {
            ...
            "fillups": "fillups",
            "fill_blank": "fillups",
            "fill_in_blanks": "fillups",
            "Fill in the Blanks":"fillups",
            "short": "short",
            "short_answer": "short",
            "Short Answer": "short",
            "long": "long",
    ...
    def _save_current_answer(self, *, push_to_server: bool = False, enforce_jit_confidence: bool = True, action: str = "save"):
        ...
        # Store answer if provided (thread-safe)
        if ans not in (None, "", []):
        ...
        # Server-driven non-JIT flows may push empty answers to support skip behavior.
        should_push_to_server = push_to_server
        if push_to_server:
            # Empty answers are still pushed for server-driven non-JIT flows.
        ...
        if should_push_to_server:
            if q_id is not None:
                ...
                # Response processing continues asynchronously.
            ...
            try:
                ...
                if self._is_server_driven_exam:
                    try:
                        ...
                        print(f"[ExamScreen] Failed to emit server response: {e}")
            except Exception as e:
                print(f"[ExamScreen] Background answer sync failed: {e}")
                
        thread = threading.Thread(target=_send, daemon=True, name="answer-persist")
        thread.start()
        return None
    ...
    def _apply_server_next_question(self, response_payload):
        ...
        # Optional evaluation feedback from server response.
        ...
        # Optional final report metadata for completed exam/section.
        if self._jit_exam_complete:
    ...
    def _on_coding_answer_submitted(self, code: str):
        ...
        try:
            ...
            if isinstance(parsed, dict) and "code" in parsed:
                parsed_code = str(parsed.get("code") or "")
                coding_index = int(parsed.get("question_index") or 0)
                coding_question_id = parsed.get("question_id")
        ...
        sec_idx = self._current_section_index
        coding_q_idx = self._current_question_index if self._is_server_driven_exam else coding_index
        if coding_q_idx < 0:
            coding_q_idx = 0

        section_questions = []
        try:
            section_questions = list(self._exam_data.get("sections", [])[sec_idx].get("questions", []) or [])
        except Exception:
            section_questions = []

        # Prefer explicit question ID from coding payload when available.
        if coding_question_id is not None and section_questions:
            target_id = str(coding_question_id)
            for idx, q in enumerate(section_questions):
                if str(q.get("id") or "") == target_id:
                    coding_q_idx = idx
                    break

        # If only coding_index exists, map it to the nth coding question in section.
        if coding_question_id is None and section_questions and not self._is_server_driven_exam:
            coding_positions = [
                i for i, q in enumerate(section_questions)
                if self._resolve_question_type(q) == "coding"
            ]
            if 0 <= coding_index < len(coding_positions):
                coding_q_idx = coding_positions[coding_index]
    ...
    def show_coding_view(self, section_index: int | None = None):
        ...
        if coding_questions:
            # Keep active coding question first so JIT next-question display remains stable.
            active_qid = None
            if 0 <= self._current_question_index < len(raw_questions):
                active = raw_questions[self._current_question_index]
                if self._resolve_question_type(active) == "coding":
                    active_qid = str(active.get("id") or "")

            if active_qid:
                prioritized = [q for q in coding_questions if str(q.get("id") or "") == active_qid]
                others = [q for q in coding_questions if str(q.get("id") or "") != active_qid]
                coding_questions = prioritized + others

            self.coding_section.set_questions(coding_questions)
    ...
    def update_violation_count(self, count: int):
        if self._violation_count == count:
            return
    ...
    def show_warning(self, message: str):
        ...
        # Each incoming alert gets a fresh visibility window.
    ...
    def _log_question_activity(
        ...
        *,
        ...
        # Log asynchronously to avoid UI blocking.
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        try:
            ...
            self._alert_timer.stop()
            self._warning_hide_timer.stop()
            self._cleanup_alert_worker(wait_ms=500)
        except Exception:
            pass
